...
TRAIN_ACC_HISTORY = 'resnet50_train_acc_history.txt'
TRAIN_LOSS_HISTORY = 'resnet50_train_loss_history.txt'
VAL_ACC_HISTORY = 'resnet50_val_acc_history.txt'
VAL_LOSS_HISTORY = 'resnet50_val_loss_history.txt'
LEARNING_RATE = 1e-4
EPOCHS = 100
SPLIT=4
...
class Trainer():
    ...
    def fit(self):
        ...
        for self.epoch in epochs:
            ...
            self.save_checkpoint(os.path.join(self.output_dir, 'resnet50_epoch{}_validloss{}_checkpoint.pth'.format(self.epoch, valid_loss)))
...
def main():
    ...
    optimizer_SGD = torch.optim.SGD(resnetModel.parameters(), lr=LEARNING_RATE)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print("Device: ", device)
    trainer = Trainer(resnetModel, optimizer_SGD, train_loader, val_loader, device=device, num_epochs=EPOCHS, output_dir=OUTPUT_DIR)
    trainer.fit()
