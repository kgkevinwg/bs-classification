...
def _efficientnet(arch, pretrained, progress, num_classes=7, **kwargs):
    width_mult, depth_mult, _, dropout_rate = params[arch]
    model = EfficientNetOriginal(width_mult, depth_mult, dropout_rate, num_classes= num_classes, **kwargs)
    if pretrained:
        print("[INFO] using pretrained network")
        ...
        print("[INFO] using scratch network")
    return model
...
def efficientnet_b4(pretrained=False, progress=True, num_classes=7, **kwargs):
    return _efficientnet('efficientnet_b4', pretrained, progress, num_classes = num_classes, **kwargs)
